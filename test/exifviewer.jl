@testset "exifviewer.jl" begin
    ...
    @testset "Different IFDs" begin
        ...
        @test length(read_tags(filepath; tags = [FLASH_PIX_VERSION, ORIENTATION], ifds = 1),) == 1
        @test length(read_tags(filepath; tags = [FLASH_PIX_VERSION, ORIENTATION], ifds = 1:4),) == 2
        @test length(read_tags(filepath; tags = [FLASH_PIX_VERSION, ORIENTATION], ifds = 4:5),) == 0
        @test length(read_tags(filepath; tags = [FLASH_PIX_VERSION, ORIENTATION], ifds = (1, 2, 3)),) == 2
        @test length(read_tags(filepath; tags = [FLASH_PIX_VERSION, ORIENTATION], ifds = (4, 5)),) == 0
        @test length(read_tags(filepath; tags = [FLASH_PIX_VERSION, ORIENTATION], ifds = 1)) == 1
        @test length(read_tags(filepath; tags = [FLASH_PIX_VERSION, ORIENTATION], ifds = (1, 2, 3))) == 2
